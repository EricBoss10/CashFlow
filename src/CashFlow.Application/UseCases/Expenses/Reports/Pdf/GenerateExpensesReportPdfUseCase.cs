...
public class GenerateExpensesReportPdfUseCase : IGenerateExpensesReportPdfUseCase
{
    ...
    private const int HEIGHT_ROW_EXPENSE_ROW = 25;
    ...
    public async Task<byte[]> Execute(DateOnly month)
    {
        ...
        var document = CreateDocument(loggerUser.Name,month);
        var page = CreatePage(document);

        CreateHeaderWithProfilePhotoAndName(loggerUser.Name,page);
        ...
        foreach(var expense in expenses)
        {
            ...
            row.Height = HEIGHT_ROW_EXPENSE_ROW;
            ...
            SeyStyleBaseForExpenseInformation(row.Cells[0]);

            row.Cells[0].Format.LeftIndent = 20;

            row.Cells[1].AddParagraph(expense.Date.ToString("t"));
            SeyStyleBaseForExpenseInformation(row.Cells[1]);

            row.Cells[2].AddParagraph(expense.PaymentType.PaymentTypeToString());
            SeyStyleBaseForExpenseInformation(row.Cells[2]);
            ...
            if(string.IsNullOrWhiteSpace(expense.Description) == false)
            {
                ...
                descriptionRow.Height = HEIGHT_ROW_EXPENSE_ROW;
    ...
    private void CreateHeaderWithProfilePhotoAndName(string name, Section page)
    {
        ...
        var pathFile = Path.Combine(directoryName!, "Logo", "black-goku.jpg");
    ...
    private void SeyStyleBaseForExpenseInformation(Cell cell)
    {
